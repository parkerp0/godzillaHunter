...
float imu_getHeading() {
    float heading;
     ...
     int16_t x = magReadings[0] + (magReadings[1] << 8);
     int16_t y = magReadings[2] + (magReadings[3] << 8);

     if(y > 0)
     {
         heading = 90.0 - (atanf((float)x/y) * 180.0f/M_PI);
     }else if(y < 0)
     {
         heading = 270.0 - (atanf((float)x/y) * 180.0f/M_PI);
     }else if(y == 0)
     {
         heading = (x < 0)? 180 : 0;
     }
