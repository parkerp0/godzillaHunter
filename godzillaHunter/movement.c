...
double move_forwardTest(oi_t *sensor_data, double distance_mm)
{
    ...
    while(sum <= distance_mm)
    {
        ...
        // breaks function if a sensor is triggered
    ...
    oi_setWheels(0,0); //stops the bot
...
void manuever(oi_t *sensor_data, double distance_mm){
    double distance = 0;
    oi_update(sensor_data);

    while (distance < distance_mm){
        oi_update(sensor_data);
        distance += move_forwardTest(sensor_data, distance);
        ...
        if (sensor_data->bumpLeft && sensor_data->bumpRight){
            ...
            distance += move_forwardTest(sensor_data, 250);
        }
        ...
        //case for hitting a cliff on the left
            ...
            turn_right(sensor_data,90);
            distance += move_forwardTest(sensor_data,300);
            turn_left(sensor_data,90);
        }
        ...
        //case for hitting the boundary on the front left
